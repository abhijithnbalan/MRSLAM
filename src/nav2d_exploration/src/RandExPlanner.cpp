// RandEx Exploration Planner
// Detailed description in:
// Bautin, A., & Simonin, O. (2012). RandEx : A Novel Frontier Allocation Algorithm for Multi-robot Exploration. ICIRA, 496–508.
#include <stdio.h>
#include <stdlib.h>
#include <algorithm>
#include <math.h>
...
int RandExPlanner::findExplorationTarget(GridMap* map, unsigned int start, unsigned int &goal)
{
	...
	ROS_DEBUG("[RandEx] Found %d frontier cells in %d frontiers.", mFrontierCells, (int)mFrontiers.size());
	if(mFrontiers.size() == 0)
	{
		...
		{
			ROS_WARN("[RandEx] No Frontiers found after checking %d cells!", cellCount);
			return EXPL_FAILED;
		}
	}
		...
		for(unsigned int i = 0; i < mFrontiers.size(); i++)
		{
			...
			for(unsigned int j = 0; j < mFrontiers[i].size(); j++)
			{
				if(p < mFrontierCells)
				{
					...
					{
						ROS_ERROR("[RandEx] getCoordinates failed!");
						break;
					}
				...
				{
					ROS_ERROR("[RandEx] SecurityCheck failed! (Asked for %d / %d)", p, mFrontierCells);
				}
				p++;
			}
		}
	...
	// 		ROS_DEBUG("[RandEx] Inserted robot at index %d.", robot);
	// 	}else
	// 	{
	// 		ROS_WARN("[RandEx] Couldn't get index of robot %d!", p->first);
	// 	}
	// }
	// ROS_DEBUG("[RandEx] Using known positions of %d other robots.", (int)indices.size());
	// std::cout<<"Exploration planning starts\n";
	
	std::vector<float> CumulatedWeight;
	float weight;
	float prev_cumu_sum = 0.0;
	for(unsigned int frontier = 0; frontier < mFrontiers.size(); frontier++)
	{
		...
		ROS_DEBUG("[RandEx] Frontier %d has rank %d and distance %.2f.", frontier, rank, distanceToRobot);
		...
		if(distanceToRobot != 0)
			{weight = fabs(mFrontiers[frontier].size()	);}
			
		else
			{weight = 1000;}
			
		prev_cumu_sum += weight;
		CumulatedWeight.push_back(prev_cumu_sum);
		// }
		
		delete[] plan;
		// if(bestRank == 0) break; // Frontiers are ordered by distance, so this will not improve anymore.
	}
	// std::cout<<"best frontier found\n";
	int biggest_cumu=0;

	// min_ele = std::min_element(CumulatedWeight.begin(),CumulatedWeight.end());
	for(int i = 1; i< CumulatedWeight.size();i++)
	{
		CumulatedWeight[i] = 10.0 * CumulatedWeight[i]/CumulatedWeight[0];
		biggest_cumu = CumulatedWeight[i];
		std::cout<<" "<<CumulatedWeight[i]<<" ";
	}
	std::cout<<"\n "<<biggest_cumu<<" \n";
	// std::cout<<"Cumulated Weight Recalculated\n";
    int RandomWeight = rand() % biggest_cumu;
	int frnt;
    // std::cout<<"Random Frontier Selected\n";
    for(frnt = 0;frnt < mFrontiers.size();frnt++)
    {   
        if(CumulatedWeight[frnt] >= RandomWeight)
        break;
    }
    // std::cout<<"\n"<<mFrontiers.size()<<" Frontiers found"<<frnt<<"th frontier selected with "<<mFrontiers[frnt].size()<<" Points!!\n";
    int bestFrontier = frnt;
	
	// 3. Assignment to a frontier
	ROS_DEBUG("[RandEx] Best frontier is %d.", bestFrontier);
	
	if(bestFrontier >= mFrontiers.size())
	{
		ROS_ERROR("[RandEx] Could not determine a best frontier. (Best: %d, Available: %d)", bestFrontier+1, (int)mFrontiers.size());
		return EXPL_FAILED;
	}
...
void RandExPlanner::findCluster(GridMap* map, unsigned int startCell)
{
...
/*
		...
		while(!unexplQueue.empty() && !isBoundary && areaSize <= minAreaSize)
		{
			...
			{
				ROS_WARN("[RandEx] Unknown cell in queue was out of map!");
				continue;
			}
	...
	ROS_DEBUG("[RandEx] Size of unknown area: %d / Boundary: %d", areaSize, isBoundary);
