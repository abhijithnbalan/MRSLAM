...
int RandExPlanner::findExplorationTarget(GridMap* map, unsigned int start, unsigned int &goal)
{
	...
	// }
    int frnt = 0;
    int TotalFrontiers = mFrontiers.size();
    float first_front_size = mFrontiers(0).size();
    std::vector CumuWeights;
    float cumu_sum = 0;
    for(int i = 0;i<TotalFrontiers;i++)
    {
        cumu_sum += mFrontiers(i).size()/first_front_size;
        CumuWeights.push_back(cumu_sum);
    }
    RandomWeight = rand()%cumu_sum;
    
    for(frnt = 0;frnt < mFrontiers.size();frnt++)
    {   
        if(CumuWeight(frnt) <= RandomWeight)
        break;
    }
