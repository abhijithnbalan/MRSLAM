...
void CommCheckCallBack	(const std_msgs::Bool::ConstPtr& dec, int index)
{

	// std::cout<<"COMM_Recheck  "<<index<<"\n";
	if(dec->data)
    {ReCheck = true;}	
    else
	{ReCheck = false;}
...
{	
	...
	// subs.push_back(robotNode.subscribe<std_msgs::Bool>("/range12", 10, boost::bind(CommCheckCallBack, _1, 1)));
	// subs.push_back(robotNode.subscribe<std_msgs::Bool>("/range23", 10, boost::bind(CommCheckCallBack, _1, 2)));
	...
	navigatorNode.param("exploration_strategy1", mExplorationStrategy, std::string("RandExPlanner"));
	navigatorNode.param("exploration_strategy", mExplorationStrategy2, std::string("NearestFrontierPlanner"));
	...
	navigatorNode.param("num_robots", NumRobots, 3);
	navigatorNode.param("robot_number", RobotNum, 1);
	...
	for(int i = 1; i <= NumRobots;i++)
	{
		...
		subs.push_back(robotNode.subscribe<std_msgs::Bool>(temp, 10, boost::bind(CommCheckCallBack, _1, i)));
	...
	// std::cout<<"Recheck topic : "<<comm_check<<"\n";
	...
	try
	{
		...
		mExplorationPlanner2 = mPlanLoader->createInstance(mExplorationStrategy2);
		ROS_INFO("Successfully loaded exploration strategy [%s].", mExplorationStrategy.c_str());
		std::cout<<"Successfully Loaded "<< mExplorationStrategy.c_str()<<"\n";
